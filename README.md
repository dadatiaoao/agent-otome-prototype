...
这是一个最小可运行的本地 Web 实验品，用来验证“6 层 Agent 架构驱动的温和协作 / 群像对话原型”。它保留亲密关系张力，但目标不是单纯攻略角色，而是观察玩家在旧书屋共创会筹备中如何分配信任、责任、可见性和边界。

核心玩法：玩家输入一句自然语言行为后，后端用 OpenAI-compatible Chat Completions API 生成三名角色对该行为的主观理解，并基于这些理解生成下一轮可见对话。连续输入 3 到 5 轮后，可以观察误解、意图、情绪、flags 和路线倾向是否被继承。

当前默认场景是海边旧书屋的春日共创会筹备：A、B、C 与玩家一起决定开场环节、读者留言墙、旧物展示、手写导览和傍晚朗读。每轮会尽量制造一个温和的小型不可兼得，例如谁的提案被放进开场、谁获得公开署名、哪些留言暂不展示、如何让某人的贡献被看见但不被推到台前。
...
玩家输入区的“AI 生成推进输入”会读取裁剪后的当前状态，并调用同一个 OpenAI-compatible API 生成一句可编辑的玩家行动建议。它不会推进回合，也不会修改状态。新建议会优先像一个人在书屋里自然做出的动作，但必须明确推进一个筹备事项，例如开场顺序、署名、留言展示范围、朗读人选或旧物摆放，同时避免任务式表态。
...
6. `memory_curator`：整理事实记忆、主观记忆、flags、角色关系状态和状态补丁。
...
3. 点击“测试模型”，确认连接成功。
4. 输入：`我把 A 的开场提案放到最前面，但请 B 公布保留意见，并让 C 决定哪些留言暂不展示。`
...
9. 观察当前状态 JSON 是否更新。
10. 再输入第二轮，观察上一轮产生的 A 的表演外壳、B 对真实判断的试探、C 对信息分配的在意等内容是否被后续继承。
